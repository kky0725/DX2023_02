...
void Maze::CreateMazeByKruskal()
{
	...
	//모든 정점에 대하여 이어져있는 간선들을 랜덤 벨류로 설정하고 edges에 추가.
	for (int y = 0; y < _poolCountY; y++)
	{
		for (int x = 0; x < _poolCountX; x++)
		{
	...
	std::sort(edges.begin(), edges.end(), [](const Edge_Kruskal& a, const Edge_Kruskal& b)-> bool {return a.cost < b.cost; });
	...
	for (auto edge : edges)
	{
		...
		if (sets.FindLaeder(u) == sets.FindLaeder(v))
			continue;
