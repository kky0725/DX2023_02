...
// DX11을 이용하여 2D 게임엔진을 만들기.

// CPU : 어려운 계산을 직렬처리한다.
// GPU : 단순 계산을 병렬 처리하는데 특화되어 있다.
// ALU가 많기 때문에 픽셀 병렬 계산에 특화되어 있다.
...
// 각각의 표면(버퍼, 텍스쳐)을 출력하기 전에 데이터를 보관하다.
...
// 렌더링파이프라인 단계
// -> 3차원에 있는 정점들을 2차원으로 투영 (Vertext Sharder)
// -> 정점들 사이에 있는 픽셀을 색을 결정 (Pixel Sharder)
...
void InitDevice()
{
    ...
    deviceContext->OMSetRenderTargets(1, renderTargetView.GetAddressOf(), nullptr); // output Merge 출력 병합
